...
const STAFF_KINDS = new Set<UploadKind>(["video", "image", "company-logo", "company-banner", "staff-avatar"]);
...
function uploadErrorStatus(message: string): number {
  if (
    ...
    message === API_ERRORS.fileRequired ||
    message === API_ERRORS.kindRequired ||
    message === API_ERRORS.invalidKind
  ) {
    return 400;
  }
  if (message === API_ERRORS.unauthorized) return 401;
  return 500;
}
...
export async function handleUploadRequest(request: Request) {
  try {
    ...
    if (!file) {
      return NextResponse.json({ error: API_ERRORS.fileRequired }, { status: 400 });
    }
    if (!kind) {
      return NextResponse.json({ error: API_ERRORS.kindRequired }, { status: 400 });
    }
    ...
    if (STAFF_KINDS.has(kind)) {
      ...
      return NextResponse.json({ error: API_ERRORS.invalidKind }, { status: 400 });
    }
    ...
    const raw = error instanceof Error ? error.message : "アップロードに失敗しました";
