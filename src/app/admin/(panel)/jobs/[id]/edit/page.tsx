...
export default function EditJobPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      let videoUrl = (form.get("videoUrl") as string) || job.videoUrl;
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        <div className="grid gap-4 md:grid-cols-2">
          <label className="block">
            ...
            <input name="title" required defaultValue={job.title} className="input-field" />
          </label>
          <label className="block">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <label className="block">
            ...
            <input name="location" required defaultValue={job.location} className="input-field" />
          </label>
          <label className="block">
            <span className="mb-1.5 block text-sm text-[#64748B]">エリア *</span>
            <select name="area" required defaultValue={job.area} className="input-field">
              {AREAS.map((a) => (
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <label className="block">
            ...
            <input name="salary" required defaultValue={job.salary} className="input-field" />
          </label>
          <label className="block">
            <span className="mb-1.5 block text-sm text-[#64748B]">職種 *</span>
            <select name="category" required defaultValue={job.category} className="input-field">
        ...
        <label className="block">
          ...
          <select name="employmentType" defaultValue={job.employmentType} className="input-field">
        ...
        <label className="block">
          ...
          <textarea name="description" required rows={4} defaultValue={job.description} className="input-field resize-none" />
        </label>
        ...
        <label className="block">
          ...
          <input name="tags" defaultValue={tagsStr} className="input-field" />
        </label>

        <div className="rounded-2xl border border-[#E2E8F0] bg-[#F8FAFC] p-5">
          <h3 className="mb-3 font-medium text-[#1E293B]">リンク設定</h3>
          <div className="grid gap-3 md:grid-cols-2">
            <input name="website" defaultValue={links.website ?? ""} className="input-field" placeholder="企業HP URL" />
            <input name="careersPage" defaultValue={links.careersPage ?? ""} className="input-field" placeholder="採用ページ URL" />
            <input name="twitter" defaultValue={links.twitter ?? ""} className="input-field" placeholder="Twitter / X URL" />
            <input name="instagram" defaultValue={links.instagram ?? ""} className="input-field" placeholder="Instagram URL" />
            <input name="linkedin" defaultValue={links.linkedin ?? ""} className="input-field" placeholder="LinkedIn URL" />
            <input name="jobPdf" defaultValue={links.jobPdf ?? ""} className="input-field" placeholder="求人票PDF URL" />
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-dashed border-[#E2E8F0] bg-[#F8FAFC] p-6">
          ...
          <label className="block">
            <span className="mb-1.5 block text-sm text-[#64748B]">動画URL</span>
            <input name="videoUrl" defaultValue={job.videoUrl} className="input-field" />
          </label>
        </div>

        <button type="submit" disabled={submitting || uploading} className="btn-primary w-full py-3">
          {uploading ? "動画アップロード中..." : submitting ? "保存中..." : "変更を保存"}
        </button>
      </form>
    </>
  );
