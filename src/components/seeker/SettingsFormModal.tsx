...
export default function SettingsFormModal({ title, onClose, children }: SettingsFormModalProps) {
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className={`settings-form-modal-overlay fixed inset-0 z-50 flex items-end justify-center backdrop-blur-sm sm:items-center sm:p-4 ${
        isDark ? "settings-form-modal-overlay--dark bg-black/70" : "bg-slate-900/50"
      }`}
    >
      <motion.div
        initial={{ y: "100%" }}
        ...
        className={`settings-form-modal-panel modal-sheet p-6 ${
      ...
      >
        ...
        <div className="settings-form-modal-content">
