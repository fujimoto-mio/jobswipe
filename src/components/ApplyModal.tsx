...
function buildLoginUrl(loginPath: string, nextPath: string): string {
  ...
  // Seeker login can return to the job after auth; staff logins use their own panels.
  if (loginPath === "/login") {
    params.set("next", nextPath || "/explore");
  }
              ...
              <Formik
                ...
                onSubmit={async (values) => {
                  ...
                  try {
                    ...
                    const data = (await res.json().catch(() => ({}))) as ApplyAuthError;
                    const message =
                    ...
                    if (
                      ...
                      data.code === "wrong_role" ||
                      data.code === "profile_missing" ||
                      (res.status === 401 && data.loginPath)
                    ) {
                      clearClientSessionCache();
                      const next = pathname?.startsWith("/jobs/") ? pathname : `/jobs/${job.id}`;
                      const loginPath = data.loginPath || "/login";
                      window.location.assign(buildLoginUrl(loginPath, next));
                      return;
                    }
